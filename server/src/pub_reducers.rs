...
pub fn sacrifice_health_for_moon(ctx: &ReducerContext, user: user::User) -> Result<(), String> {
    ...
    let mut health_to_sacrifice = (user.size*super::PORTION_HEALTH_SACRIFICE).min(super::MAX_HEALTH_SACRIFICE);

    if !user_moon::can_get_moon_into_orbit(&user, health_to_sacrifice*super::MAX_MOON_SIZE_PER_HEALTH) { // use max here so user cant cheese to get the max every time
