    ...
    <Card className="border-4 border-zinc-800 bg-zinc-900 shadow-lg w-full h-full flex flex-col p-0 bg-zinc-950 overflow-hidden">
      <CardContent className="p-0 flex-1 flex flex-col overflow-auto max-w-full">
        <Table className="w-full">
          <TableHeader>
            <TableRow>
                <TableHead className="text-white">Rank {`(of ${leaderboardEntries.size})`}</TableHead>
            ...
            {Array.from(leaderboardEntries.values())
              ...
              .map((entry) => (
                ...
                >
                    ...
                    <TableCell className="font-medium text-zinc-300 truncate max-w-[150px]">{entry.username}</TableCell>
                    <TableCell className="text-zinc-300">{Math.round(entry.size)}</TableCell>
                    <TableCell className="text-zinc-300">{entry.kills}</TableCell>
                    <TableCell className="text-zinc-300">{Math.round(entry.damage)}</TableCell>
                </TableRow>
            ))}
          </TableBody>
        </Table>
